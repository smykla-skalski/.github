...
func TestApplyVersionReplacements(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			content:     "uses: smykla-labs/smyklot@v1.0.0",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "uses: smykla-labs/smyklot@v2.0.0",
			wantChanged: true,
			description: "Should replace GitHub Action version tag",
		},
		{
			name:        "replace Docker image version",
			content:     "image: ghcr.io/smykla-labs/smyklot:1.0.0",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "image: ghcr.io/smykla-labs/smyklot:2.0.0",
			wantChanged: true,
...
jobs:
  test:
    steps:
      - uses: smykla-labs/smyklot@v1.0.0
      - run: docker pull ghcr.io/smykla-labs/smyklot:1.0.0`,
			version: "2.0.0",
			tag:     "v2.0.0",
...
jobs:
  test:
    steps:
      - uses: smykla-labs/smyklot@v2.0.0
      - run: docker pull ghcr.io/smykla-labs/smyklot:2.0.0`,
			wantChanged: true,
			description: "Should replace both patterns in complete workflow",
		},
		{
			name:        "no change when versions already match",
			content:     "uses: smykla-labs/smyklot@v2.0.0",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "uses: smykla-labs/smyklot@v2.0.0",
			wantChanged: false,
		...
		},
		{
			...
			content:     "url: https://evil.com?ref=smykla-labs/smyklot@v1.0.0",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "url: https://evil.com?ref=smykla-labs/smyklot@v1.0.0",
			wantChanged: false,
			description: "Should NOT match pattern embedded in URL query params",
		},
		{
			name:        "security: reject malicious Docker image in URL path",
			content:     "url: https://evil.com/ghcr.io/smykla-labs/smyklot:1.0.0/malware",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "url: https://evil.com/ghcr.io/smykla-labs/smyklot:1.0.0/malware",
			wantChanged: false,
			description: "Should NOT match pattern embedded in URL path",
		},
		{
			name:        "security: reject version suffix bypass",
			content:     "uses: smykla-labs/smyklot@v1.0.0-malicious",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "uses: smykla-labs/smyklot@v1.0.0-malicious",
			wantChanged: false,
		...
		},
		{
			...
			content:     "image: ghcr.io/smykla-labs/smyklot:1.0.0-alpine",
			version:     "2.0.0",
			tag:         "v2.0.0",
			wantContent: "image: ghcr.io/smykla-labs/smyklot:1.0.0-alpine",
			wantChanged: false,
